...
import { mockCompanies, createMockResponse, mockApiDelay } from '../../services/mockData'
...
export const fetchCompanies = createAsyncThunk(
  ...
  async (params, { rejectWithValue }) => {
    try {
      ...
      if (params?.search) {
        filteredData = mockCompanies.filter(company =>
          company.company_name.toLowerCase().includes(params.search.toLowerCase()) ||
          company.email.toLowerCase().includes(params.search.toLowerCase())
        )
      }
      
      if (params?.status) {
        filteredData = filteredData.filter(company => company.status === params.status)
      }
      
      if (params?.subscription_plan) {
        filteredData = filteredData.filter(company => company.subscription_plan === params.subscription_plan)
      }
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchCompanies.fulfilled, (state, action) => {
        state.loading = false
        state.companies = action.payload.companies || action.payload.data || action.payload
...
export const { setFilters, clearFilters, setPagination, clearError } = companySlice.actions
...
export const selectCompanyPagination = (state) => state.company?.pagination || initialState.pagination
export const selectCompanyFilters = (state) => state.company?.filters || initialState.filters
