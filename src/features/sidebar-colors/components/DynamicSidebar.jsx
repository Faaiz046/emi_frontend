...
const DynamicSidebar = ({ selectedColorScheme = 'blue' }) => {
  ...
  const colorSchemes = {
    ...
    blueToPurple: 'from-blue-500 to-purple-600',
    indigoToPink: 'from-indigo-500 to-pink-600',
    emeraldToTeal: 'from-emerald-500 to-teal-600',
    roseToOrange: 'from-rose-500 to-orange-500',
    violetToPurple: 'from-violet-500 to-purple-600',
    cyanToBlue: 'from-cyan-500 to-blue-600',
    greenToEmerald: 'from-green-500 to-emerald-600',
    yellowToOrange: 'from-yellow-500 to-orange-500',
    darkBlue: 'from-blue-800 to-blue-900',
    darkPurple: 'from-purple-800 to-purple-900',
    darkSlate: 'from-slate-800 to-slate-900',
    darkGray: 'from-gray-800 to-gray-900',
    goldToAmber: 'from-yellow-400 to-amber-500',
    silverToGray: 'from-gray-300 to-gray-400',
    copperToOrange: 'from-orange-400 to-red-500',
  ...
  const getSidebarClasses = () => {
    const baseClasses = 'transition-all duration-300 ease-in-out backdrop-blur-sm shadow-lg border-r border-white/10';
    const selectedGradient = colorSchemes[selectedColorScheme] || colorSchemes.blue;
    
    switch (layoutType) {
      case LAYOUT_TYPES.CLASSIC:
        return `${baseClasses} ${sidebar.isOpen ? 'w-64' : 'w-16'} bg-gradient-to-b ${selectedGradient}`;
      case LAYOUT_TYPES.MODERN:
        return `${baseClasses} ${sidebar.isOpen ? 'w-72' : 'w-20'} bg-gradient-to-b ${selectedGradient}`;
      case LAYOUT_TYPES.STACKED_SIDE:
        return `${baseClasses} w-full bg-gradient-to-b ${selectedGradient}`;
      case LAYOUT_TYPES.DECKED:
        return `${baseClasses} lg:col-span-3 bg-gradient-to-b ${selectedGradient}`;
      case LAYOUT_TYPES.SIMPLE:
        return `${baseClasses} w-64 bg-gradient-to-b ${selectedGradient}`;
      case LAYOUT_TYPES.COMPACT:
        return `${baseClasses} ${sidebar.isOpen ? 'w-48' : 'w-12'} bg-gradient-to-b ${selectedGradient}`;
      case LAYOUT_TYPES.FLUID:
        return `${baseClasses} ${sidebar.isOpen ? 'w-80' : 'w-24'} bg-gradient-to-b ${selectedGradient}`;
      default:
        return `${baseClasses} ${sidebar.isOpen ? 'w-64' : 'w-16'} bg-gradient-to-b ${selectedGradient}`;
    }
  };
  ...
  const getNavItemClasses = (isActive = false) => {
    const baseClasses = 'flex items-center px-4 py-3 rounded-xl transition-all duration-200 font-medium text-sm relative';
  ...
  const getSubmenuItemClasses = (isActive = false) => {
    const baseClasses = 'flex items-center px-4 py-2.5 rounded-lg transition-all duration-200 font-medium text-sm ml-8';
  ...
  const renderNavItem = (item, index = 0) => {
        ...
        <div key={item.id} className="relative">
          <div className="relative">
            <button
              ...
              onMouseEnter={() => !sidebar.isOpen && setHoveredItem(`menu-${index}`)}
          ...
          {item.isOpen && sidebar.isOpen && item.items && isAnimationComplete && (
          ...
          {!sidebar.isOpen && (hoveredItem === `menu-${index}` || item.isOpen) && isAnimationComplete && (
            <div
                ...
                return (
                  <a
                    ...
                    className={`flex items-center gap-3 px-4 py-2 rounded-lg text-sm transition-all ${
                      subIsActive ? 'bg-blue-50 text-blue-600 shadow-sm' : 'hover:bg-gray-50'
                    }`}
                  >
                    {SubIconComponent && (
                      <SubIconComponent className={`w-3.5 h-3.5 ${subIsActive ? 'text-blue-600' : 'text-gray-500'}`} />
                    )}
    ...
    return (
      <a
        ...
        onMouseLeave={() => setHoveredItem(null)}
        className={`${getNavItemClasses(isActive)} ${!sidebar.isOpen && layoutType !== LAYOUT_TYPES.STACKED_SIDE ? 'justify-center' : ''}`}
      >
        {IconComponent && (
          <IconComponent className={getIconClasses(isActive)} />
        )}
  ...
  return (
    <aside className={`h-screen flex flex-col justify-between z-50 ${getSidebarClasses()}`}>
        ...
        <div className="p-4 space-y-3 border-t border-white/10 relative z-10">
          <div className="px-4 py-4 bg-white/10 backdrop-blur-sm rounded-xl mb-3">
            ...
            <div className="flex items-center justify-between text-xs">
              <span className="cursor-pointer transition-colors text-white/60 hover:text-white">Privacy</span>
              <span className="cursor-pointer transition-colors text-white/60 hover:text-white">Terms</span>
            </div>
          </div>
          ...
          <a
            ...
            onMouseLeave={() => setHoveredItem(null)}
            className="relative flex items-center gap-3 p-3 rounded-xl hover:bg-white/10 transition-all text-white"
          >
          ...
          <button
            ...
            onMouseLeave={() => setHoveredItem(null)}
            className="relative w-full flex items-center gap-3 p-3 rounded-xl text-red-300 hover:bg-red-500/20 hover:shadow-sm transition-all"
          >
            <ArrowRightOnRectangleIcon className="w-5 h-5" />
            {sidebar.isOpen && <span className="text-sm font-medium">Logout</span>}
